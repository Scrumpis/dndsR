...
#' @param exclude_terms Character vector of term types to exclude everywhere
...
term_enrichment <- function(dnds_annot_file = NULL,
  ...
  .available_terms <- function(df) {
    # Keep a conservative allowlist for detection; custom terms still work if in q_*/s_*
    allowed <- c("kegg","panther","pfam","tigrfam","cog","ipr","go")
    have <- function(suf) {
  ...
  .detect_term_columns <- function(d) {
    # Detect any q_*/s_* string columns as potential term families,
    # then harmonize names (strip q_/s_) and deduplicate.
    qcols <- names(d)[startsWith(names(d), "q_")]
    scols <- names(d)[startsWith(names(d), "s_")]
    cand <- unique(tolower(sub("^q_|^s_", "", c(qcols, scols))))
    # Keep only those whose corresponding columns are character vectors
  ...
  .run_one_comparison <- function(comp_name, comp_dir) {
    ...
    # Auto-detect: start from known allowlist + any custom q_*/s_* columns
    base_detect <- .available_terms(d)
    term_set <- base_detect

    # Apply exclusions for auto-detect
    if (!is.null(exclude_terms) && length(exclude_terms)) {
      term_set <- setdiff(term_set, tolower(exclude_terms))
    }

    # If user supplied `terms=`, use it but still respect exclusions (unless exclude_terms=NULL)
    if (!is.null(terms) && length(terms)) {
      term_set <- tolower(terms)
      if (!is.null(exclude_terms) && length(exclude_terms)) {
        drop_now <- intersect(term_set, tolower(exclude_terms))
        if (length(drop_now)) {
          message("Dropping excluded terms: ", paste(drop_now, collapse = ", "))
  ...
  # Auto-detect + custom + exclusions in single mode too
