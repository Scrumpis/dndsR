#' Append annotations from query & subject GFFs to dN/dS results
#'
...
#'
#' @param dnds_file Path to a dN/dS TSV (single mode). Must contain columns
#'   \code{query_id} and \code{subject_id}.
#' @param query_gff Path to a query GFF3 file (single mode).
#' @param subject_gff Path to a subject GFF3 file (single mode).
#' @param output_file Optional output path (single mode). If \code{NULL}, returns
#'   the annotated data.frame instead of writing to disk.
#' @param comparison_file Path to a whitespace-delimited file (tabs/spaces; header or not)
...
#' @param custom Optional comma-separated patterns like \code{"REX{5},TOM{8}"} meaning
#'   PREFIX followed by exactly N digits; produces \code{q_<prefix>} / \code{s_<prefix>}
#'   columns (lowercased) when present.
#' @param threads Integer threads to use for per-ID annotation. If \code{NULL}/\code{NA},
#'   uses \code{options(dndsR.threads)} or \code{DNDSR_THREADS}; defaults to 1.
#' @param overwrite Logical; in batch mode, skip a comparison if output exists unless
...
#'
#' @return In single mode: a data.frame (unless \code{output_file} is provided, in which
