...
#' @param strict_coverage Numeric in [0,1]; fail if best coverage < this (default 0.90).
    ...
    if (!is.null(stratified_types)) {
      for (tp in stratified_types) {
        ...
        if (method == "fisher") {
          ...
          else message(sprintf("[ipr_enrichment] No rows for type=%s side=%s", tp, side))
        } else {
          ...
          else message(sprintf("[ipr_enrichment] No rows (hierarchy mode) for type=%s side=%s", tp, side))
        }
      }
      return(results)
    }
